...
function set_test_config {
	...
	;;
    riscv32-ilp32d)
        test_defconfig="qemu_riscv32_virt_defconfig"
        test_board_dir="riscv32-virt"
        ;;
    riscv64-lp64d)
...
function build_test {
    ...
    echo "BR2_WGET=\"wget --retry-on-host-error -nd -t 10 --no-check-certificate\"" >> ${test_build_dir}/br_fragment
